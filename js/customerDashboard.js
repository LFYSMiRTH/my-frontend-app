  ...
  document.querySelector('.bell')?.addEventListener('click', async () => {
    try {
      ...
      if (orderNotifs.length === 0) {
        showAlert('No order notifications', 'No new order updates.');
        return;
      }
      let msg = '';
      ...
      showAlert(
        ...
        async () => {
          try {
            ...
            showAlert('Error', 'Failed to clear notifications: ' + err.message);
          }
        }
      );
    } catch (err) {
...
async function loadAllMenuItemsForCarousel() {
  ...
  try {
    ...
    showAlert('Error', `Failed to load menu items for carousel: ${err.message}`);
...
async function setupDeleteConfirmation() {
  ...
  confirmDeleteBtn.addEventListener('click', async function() {
    ...
    try {
      showAlert(
        ...
        async () => {
          try {
            ...
            localStorage.removeItem('customerInfo');
            localStorage.removeItem('tambayanCart');
            showAlert('Account Deleted', 'Your account has been successfully deleted. You will be redirected to the login page.');
            setTimeout(() => {
              window.location.href = '/html/login.html';
            }, 2000);
          } catch (err) {
            showAlert('Delete Failed', 'Account deletion failed: ' + err.message);
          }
        },
        () => {}
      );
      ...
      showAlert('Error', 'Failed to verify password: ' + err.message);
    }
  });
...
function handleProfilePictureUpload(event) {
  ...
  if (!file.type.startsWith('image/')) {
    showAlert('Invalid File', 'Please select an image file (JPEG, PNG, GIF, etc.)');
    return;
  }
